...
def build_regression_data(lookback_days=1, gpd_method="1pt", test_exp_dates=None):
    ...
    # 儲存 RND 統計資料
    output_rnd = f"output/RND_stats_{gpd_method}_{lookback_days}d_{today}.csv"
    ...
    # 儲存最終結果
    output_all = f"output/RND_regression_all_{gpd_method}_{lookback_days}d_{today}.csv"
...
if __name__ == "__main__":
    ...
    使用單筆資料測試基礎計算及繪圖
    """
    observation_date = "2020-03-13"
    expiration_date = "2020-03-20"
    call_iv, put_iv, call_price, put_price, df_idx = read_data(expiration_date)
    F = find_F(observation_date, expiration_date, call_iv, put_iv, call_price, put_price, df_idx)
    basicinfo = get_FTS(observation_date, expiration_date, call_iv, put_iv, call_price, put_price, df_idx)
    df_options_mix, basicinfo = mix_cp_function(observation_date, expiration_date, call_iv, put_iv, call_price, put_price, df_idx)
    plot_implied_volatility(df_options_mix, basicinfo, observation_date, expiration_date)
    smooth_IV = UnivariateSpline_function(df_options_mix, basicinfo, power=4, s=None, w=None)
    fit = RND_function(smooth_IV, df_options_mix)
