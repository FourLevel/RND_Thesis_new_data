...
TARGET_COL = "T Return"
CORE_FACTORS = ["Skewness", "Kurtosis"]
...
def screen_candidate_models(
    ...
    for core_size in range(1, len(core_factors) + 1):
        for core_combo in itertools.combinations(core_factors, core_size):
            for k in range(0, control_cap + 1):
                for combo in itertools.combinations(control_vars, k):
                    features = list(core_combo) + list(combo)
                    ...
                    core_ps = [pvalues.get(var, np.nan) for var in core_combo]
                    control_ps = [pvalues.get(var, np.nan) for var in combo]

                    pass_core = any(pd.notna(p) and p < P_THRESHOLD_REQUIRED for p in core_ps)
                    pass_controls = all(pd.notna(p) and p < P_THRESHOLD_CONTROLS for p in control_ps)
                    passed = pass_core and pass_controls
                    ...
                    rows.append(
                        {
                            ...
                            "aic": result.aic,
                            "bic": result.bic,
                            "core_factors": " + ".join(core_combo),
                            "core_factor_min_pvalue": np.nanmin(core_ps) if len(core_ps) else np.nan,
...
def evaluate_single_model_task(model_row: dict, merged: pd.DataFrame, merged_screening: pd.DataFrame) -> tuple[dict | None, pd.DataFrame | None]:
    ...
    summary_row = {
        ...
        "screening_core_factors": model_row["core_factors"],
        "screening_core_factor_min_pvalue": model_row["core_factor_min_pvalue"],
...
def main() -> None:
    ...
    models_df = models_df.sort_values(
        ["pass_pvalue_screen", "core_factor_min_pvalue", "adj_rsquared"],
    ...
    if summary_rows:
        ...
        summary_df = pd.DataFrame(
            columns=[
                ...
                "variables",
                "num_variables",
                "screening_core_factors",
                "screening_core_factor_min_pvalue",
                "final_filter_rule",
                "n_obs_after_filter",
                "retention_ratio",
                "n_oos",
