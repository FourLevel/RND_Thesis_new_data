...
DATA_1PT_PATH = Path("output/oos_sample_selection_1d/model_0522_1pt_1d_filtered.csv")
DATA_2PT_PATH = Path("output/oos_sample_selection_1d/model_0522_2pt_1d_filtered.csv")
OUTPUT_DIR = Path("output/oos_sample_selection_1d/model_0522_analysis")

DATE_COL = "Observation Date"
TARGET_COL = "T Return"
SELECTED_VARS = ["Skewness", "Kurtosis", "T-4 Return"]
...
def main() -> None:
    ...
    descriptive_stats(df_1pt).to_csv(
        OUTPUT_DIR / "model_0522_1pt_descriptive_stats_1d.csv",
        index=False,
        encoding="utf-8-sig",
    )
    descriptive_stats(df_2pt).to_csv(
        OUTPUT_DIR / "model_0522_2pt_descriptive_stats_1d.csv",
        index=False,
        encoding="utf-8-sig",
    )

    correlation_matrix(df_1pt).to_csv(
        OUTPUT_DIR / "model_0522_1pt_correlation_matrix_1d.csv",
        encoding="utf-8-sig",
    )
    correlation_matrix(df_2pt).to_csv(
        OUTPUT_DIR / "model_0522_2pt_correlation_matrix_1d.csv",
        encoding="utf-8-sig",
    )

    reg_table = build_regression_oos_table(df_1pt, df_2pt)
    reg_table.to_csv(
        OUTPUT_DIR / "model_0522_regression_oos_comparison_1d.csv",
        index=False,
        encoding="utf-8-sig",
    )
